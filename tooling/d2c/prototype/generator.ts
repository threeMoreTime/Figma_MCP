/**
 * React Prototype Generator (Phase 5A)
 *
 * Generates interactive React Prototype in examples/prototype/users/
 * from Phase 4 outputs.
 */

import { writeFileSync, mkdirSync } from "node:fs";
import { resolve } from "node:path";
import type { PrototypeSpec } from "./adapter.js";

export function generatePrototypeFiles(spec: PrototypeSpec, targetDir: string) {
  mkdirSync(targetDir, { recursive: true });

  // 1. Generate prototype.css (Strictly semantic CSS variables, zero hardcoding)
  const cssContent = `/**
 * Prototype Stylesheet (Phase 5A)
 * Strictly uses semantic CSS variables from canonical tokens.
 * Zero hardcoded colors (#1677ff) or spacing (16px).
 */

.prototype-container {
  padding: var(--d2c-spacing-lg);
  background-color: var(--d2c-color-bg-layout);
  min-height: 100vh;
  box-sizing: border-box;
}

.prototype-header-region {
...
.prototype-content-region {
  background-color: var(--d2c-color-bg-container);
  padding: var(--d2c-spacing-md);
  border-radius: var(--d2c-borderRadius-base);
  border: 1px solid var(--d2c-color-border);
}
...
.state-switcher-bar {
  ...
  align-items: center;
  gap: var(--d2c-spacing-sm);
  padding: var(--d2c-spacing-sm) var(--d2c-spacing-md);
  background-color: var(--d2c-color-neutral-white);
  border-bottom: 1px dashed var(--d2c-color-border);
  margin-bottom: var(--d2c-spacing-md);
  border-radius: var(--d2c-borderRadius-base);
}
...
export const UsersPrototype: React.FC = () => {
...
`;
  ...
  const cssPath = resolve(targetDir, "prototype.css");
  const tsxPath = resolve(targetDir, "UsersPrototype.tsx");
  const indexPath = resolve(targetDir, "index.tsx");

  writeFileSync(cssPath, cssContent, "utf-8");
  writeFileSync(tsxPath, tsxContent, "utf-8");
  writeFileSync(indexPath, indexContent, "utf-8");

  console.log(`✓ Prototype files generated in: ${targetDir}`);
}
